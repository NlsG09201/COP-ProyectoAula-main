...
let OdontogramaPageComponent = class OdontogramaPageComponent {
    constructor(api) {
        this.api = api;
        this.superiores = [];
        this.inferiores = [];
    ...
    toggle(p) { p.seleccionada = !p.seleccionada; }
    aplicarASeleccion() {
        [...this.superiores, ...this.inferiores].forEach(p => { if (p.seleccionada) {
            p.estado = this.estado;
            p.observacion = this.observacion;
        } });
    }
    guardar() {
        if (!this.pacienteId || !this.fecha) {
            this.msg = 'Paciente y fecha son obligatorios.';
            return;
        }
        const seleccionadas = [...this.superiores, ...this.inferiores].filter(p => p.estado);
        if (!seleccionadas.length) {
            this.msg = 'Selecciona al menos un diente con estado.';
            return;
        }
        this.loading = true;
        this.msg = '';
        ...
        this.api.post('/odontogramas', odontogramaBody).subscribe({
            next: (od) => {
                const idO = od?.idOdontograma;
                if (!idO) {
                    this.msg = 'Odontograma creado sin ID';
                    this.loading = false;
                    return;
                }
                ...
                const requests = seleccionadas.map(sel => {
                    ...
                    return this.api.post('/detalles-odontograma', body);
                });
                let completadas = 0;
                requests.forEach(r => r.subscribe({
                    next: () => { completadas++; if (completadas === requests.length) {
                        this.msg = 'Odontograma guardado con detalles';
                        this.loading = false;
                    } },
    ...
    cargarDientes() {
        this.api.get('/dientes').subscribe({
            next: (dientes) => {
...
OdontogramaPageComponent = __decorate([
    Component({
        standalone: true,
...
], OdontogramaPageComponent);
export { OdontogramaPageComponent };
//# sourceMappingURL=odontograma-page.component.js.map