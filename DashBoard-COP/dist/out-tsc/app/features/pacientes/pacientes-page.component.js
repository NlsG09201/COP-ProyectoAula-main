...
let PacientesPageComponent = class PacientesPageComponent {
    constructor(api) {
        this.api = api;
    ...
    guardar() {
        if (!this.form.docIden || !this.form.nombreCompleto) {
            this.msg = 'Documento y Nombre son obligatorios.';
            return;
        }
        this.loading = true;
        this.msg = '';
        this.api.post('/pacientes', this.form).subscribe({
    ...
    aplicarFiltro() {
        ...
        this.filtrados = this.pacientes.filter(p => ((p.docIden || '').toLowerCase().includes(t) || (p.nombreCompleto || '').toLowerCase().includes(t)));
    }
};
PacientesPageComponent = __decorate([
    Component({
        standalone: true,
...
export { PacientesPageComponent };
//# sourceMappingURL=pacientes-page.component.js.map